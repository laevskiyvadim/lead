...
import Link from "@/components/app/Nav/Link";
import LinkSubLinks from "@/components/app/Nav/LinkSubLinks";
...
export default {
  ...
  setup(props) {
    ...
    const linkOrSubLinks = (link) =>
      h(
        link.items
          ? h(<LinkSubLinks link={link} toggleClass={toggleClass} />)
          : h(<Link link={link} />)
      );
    const liCls = ({ items }) => {
      return {
...
.menu {
  ...
  grid-template-rows: 97px 72px 1fr;
}
.menu.active {
  width: 45px;
  display: grid;
